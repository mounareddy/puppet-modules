class cm::cluster_template (
  ...
  $hive_metastore_password = $cm::params::hive_metastore_password,
    ...
    exec { "importing the cluster to a new cluster"
      command => "curl -X POST -H "Content-Type: application/json" -d @kerberized_cluster_template.json http://${cm_username}:${cm_password}@${cm_hostname}:${cm_port}/api/v12/cm/importClusterTemplate",
      cwd     => '/root',
      path    => '/bin',
