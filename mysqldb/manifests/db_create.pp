...
define mysqldb::db_create (String $user, String $password) {
    ...
    exec { "create-${name}-db":
      ...
      command => "/usr/bin/mysql -uroot --password=$new_password -e \"create database ${name}; grant all on ${name}.* to ${user}@localhost identified by '$password'; grant all on ${name}.* to ${user}@% identified by '$password'\"",
